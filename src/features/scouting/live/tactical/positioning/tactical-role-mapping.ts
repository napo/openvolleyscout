...
export const ROLE_SEQUENCE_COURT_POSITIONS: readonly CourtPosition[] = [1, 2, 3, 4, 5, 6];
export const SIDE_OUT_ROTATION_ORDER: readonly CourtPosition[] = [1, 6, 5, 4, 3, 2];
...
function getRoleInitialCourtPosition(roleSequence: readonly PlayerRole[], role: PlayerRole): CourtPosition | null {
  ...
  return ROLE_SEQUENCE_COURT_POSITIONS[roleIndex] ?? null;
}
...
export function getTeamRolePlayerMap({
  ...
  const roleAlignedSlots = roleSequence
    .map((role, index): RoleMappableLineupSlot | null => {
      ...
      const initialCourtPosition = ROLE_SEQUENCE_COURT_POSITIONS[index];
